...
This file is read by `/execute-task` when the task queue (built in Phase 1.1) contains more than one task. Read this after Phase 7 (context maintenance) completes for the current task.

Context from the caller: the remaining task queue, the current feature directory.

## Queue Processing

After Phase 7 completes for the current task:
